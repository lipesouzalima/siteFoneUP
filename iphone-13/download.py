...
def process_file(entry):
    ...
    new_name = filename
    new_name = re.sub(r'^iplaceprd_\d+_iphone13_', '', new_name)
    new_name = re.sub(r'iplace', '', new_name, flags=re.IGNORECASE)
    if new_name.startswith('_') or new_name.startswith('-'): new_name = new_name[1:]
    
    if not new_name.startswith('foneup-iphone13-'):
        if new_name.startswith('iphone13-'):
            new_name = 'foneup-' + new_name
        else:
            new_name = 'foneup-iphone13-' + new_name.replace('_', '-')
...
base_tag = '<base href="/iphone-13/" target="_top">'
