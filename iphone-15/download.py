...
def process_file(entry):
    ...
    new_name = filename
    new_name = re.sub(r'^iplaceprd_\d+_iphone15_', '', new_name)
    new_name = re.sub(r'iplace', '', new_name, flags=re.IGNORECASE)
    if new_name.startswith('_') or new_name.startswith('-'): new_name = new_name[1:]
    
    if not new_name.startswith('foneup-iphone15-'):
        if new_name.startswith('iphone15-'):
            new_name = 'foneup-' + new_name
        else:
            new_name = 'foneup-iphone15-' + new_name.replace('_', '-')
...
base_tag = '<base href="/iphone-15/" target="_top">'
